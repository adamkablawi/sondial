...
export async function runZooCopilot(opts: ZooRunOptions): Promise<ZooGenerationResult> {
  ...
  return new Promise<ZooGenerationResult>((resolve, reject) => {
    // Node's global WebSocket cannot set handshake headers, which is why the
    // `headers` message below exists for browsers. Server-side we authenticate
    // on the handshake instead: Zoo rejects the message-based form here.
    ...
    ws.addEventListener("open", () => {
      ...
      send({
        ...
        ...(opts.currentFiles
          ? {
              // Must be byte arrays on the wire, not strings.
              current_files: Object.fromEntries(
                Object.entries(opts.currentFiles).map(([path, body]) => [
                  path,
                  Array.from(Buffer.from(body, "utf8")),
                ]),
              ),
            }
          : {}),
    ...
    ws.addEventListener("close", (event) => {
      ...
      if (!settled) {
        fail(
          new ZooError(
            `Zoo websocket closed before finishing (code ${event.code ?? "?"})`,
            "closed",
          ),
        );
      }
